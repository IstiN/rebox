...
Package **`@rebox.me/rebox`**: use **Trusted Publishing (OIDC)** — no **`NPM_TOKEN`** needed; see **[docs/NPM-ORG.md](NPM-ORG.md)**. Run **Publish to npm** after configuring the trusted publisher on npmjs.com (workflow **`publish-npm.yml`**).
